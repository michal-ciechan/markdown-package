...
internal static class ApiSupport
{
    ...
    internal static void Creation(CreationOptions options, SnapshotMetadata? metadata, HistoryMode history)
    {
        ArgumentNullException.ThrowIfNull(options);
        if (!Enum.IsDefined(history)) throw new ArgumentException("Invalid history mode.");
        if (history == HistoryMode.None && (metadata is not null || options.ReverseIndex || options.ObjectFormat != "sha1"))
            throw new ArgumentException("Commit metadata and Git options require Git history.");
