...
- `.mdpkg/` and `.git/` are reserved. No tracked path may begin with either prefix after NFC and case folding, except the tracked addressing paths under `.mdpkg/address/` (§6.3) and the tracked review paths under `.mdpkg/review/` (§6.8). Both exceptions exist for the same reason: the data must be tracked so that its edits are commits, it has history of its own, and it survives a squash as an ordinary tree copy; and it must sit under the reserved prefix so it can never collide with a document. Git enforces `.git/` and `.GIT/` in `fsck` and checkout; it accepts `.mdpkg/x.json` without complaint, so the producer MUST enforce the `.mdpkg/` reservation itself. A producer importing a source tree that contains a real `.mdpkg/` directory MUST reject it or relocate the colliding paths and declare that it did.
...
One further object is defined and is present **only in a review package** (§6.8), where it is genuinely read before a reader can do anything else: it is how a reader tells a review package from an ordinary one and decides whether it must go and find the reviewed original.

```json
 "review":{"detail":".mdpkg/review/comments.json",
           "of":{"current":"sha1-…","dispatch":"…","namespace":"c1b2d3e4-…","packageBytes":251778,"packageDigest":"sha256-…"},
           "shape":"delta"}
```

| Field | Type | Meaning |
| --- | --- | --- |
| `review.of.namespace`, `review.of.current` | UUID, qualified object ID | The correlation key: the exact lineage state this package reviews. Both REQUIRED. A reader matches on the pair and on nothing else, exactly as §6.4 rejects a reference from a foreign namespace before resolving anything (D-25). |
| `review.of.packageDigest`, `review.of.packageBytes` | string, integer | Corroborating evidence, OPTIONAL and RECOMMENDED: the SHA-256 and length of the file the reviewer held. A match means the reviewer held those exact bytes; `current` matching while the digest does not means the same lineage state was re-emitted or repacked, which is a fact worth reporting and is not an error; if `current` does not match, the digest is irrelevant. Never identity (D-25). |
| `review.of.dispatch` | string | OPTIONAL transport metadata echoed back from the sender, stored and never interpreted. A producer that fixes commit dates can emit two byte-identical packages at different times, and no content-derived identifier can tell them apart; which *send* a review answers is transport knowledge, not package identity (D-26). |
| `review.shape` | `delta` or `bundled` | `delta` carries the review alone in the reviewer's own fresh namespace; `bundled` carries the reviewed package too and continues its lineage (D-24, §6.8). |
| `review.detail` | entry name | The tracked review document, §6.8. Normally `.mdpkg/review/comments.json`. |

The object is O(1) with a fixed key set, and it is the only manifest field whose absence is itself the answer to a question: a package without `review` is not a review package.
...
Two further obligations apply to a package carrying `review`, and both are checkable from what a validator already holds:

- A `bundled` review package's `namespace` MUST equal `review.of.namespace`, and its tip commit MUST change only paths under `.mdpkg/review/` — no document edits and no ledger edits. A validator checks the whole claim by diffing the tip tree against its parent, whose commit MUST be `review.of.current`. This is what makes it safe for a reviewer to be briefly a producer in the reviewed lineage without confirmation authority under §6.3: no addressing exception is created, every root stays valid, and the ledger is copied with the tree.
- A `delta` review package's `namespace` MUST differ from `review.of.namespace`. A delta review is the reviewer's own one-commit lineage over its own review document; the roots and digests inside it belong to the *reviewed* namespace and are read there (§6.8), and collapsing the two namespaces would make every thread a self-reference.
...
| `occurrence` | Zero-based index of `quote` among its occurrences in the scope at review time. It lets a reader tell a duplicated quote from a moved one, and it is a tiebreak of last resort, after context. |
...
**Where the anchors live.** Review data is a *second package*, never an addition to the reviewed one (D-23): writing it into the reviewed package would change the tip tree, hence `current`, hence the identity every thread points at, so the artifact would invalidate its own references on write. The review package is an ordinary package — §3 through §5 apply to it unchanged, its comment edits are commits, and it can itself be reviewed — carrying the manifest `review` object of §4 and one tracked document at `.mdpkg/review/comments.json` (§3.6). Measured price of being a package rather than a bare JSON file: a flat 2,613–2,615 bytes on npm scale regardless of thread count, which is 51% of a one-thread review and 1.9% of a 200-thread one, and it buys the whole container profile — 79-byte typing, central-directory integrity, the LF rule, the text-flag rule, and a history of its own.
...
- **The `comments` array order is authoritative and `at` is display metadata.** Two machines with skewed clocks order timestamps inconsistently, and the review package's own history already records the true order of edits. Nothing in resolution depends on a reviewer's clock.
...
**Shapes.** `delta` carries the review alone, in a fresh namespace of the reviewer's own, and is the normal case because the reviewed party already holds the original it produced; measured at 5,098 bytes for 1 thread and 37,463 for 50, against an original of 251,778 — a 50-thread review travels at 14.9% of the artifact it reviews. `bundled` carries the reviewed package too and continues its lineage, so its `current`'s parent is literally `review.of.current` and the correlation is structural rather than declared; it costs the original plus the same increment (254,262 bytes for 1 thread, 286,628 for 50) and is for the case where the reviewed party cannot be assumed to still hold `review.of.current` — an archival hand-off, a third party, or a producer that has since squashed that state out of existence. Both are first-class and the choice is the reviewer's at emit time (D-24). A thread whose reviewed state has moved on needs no new mechanics: it is a §6.4 current reference without `at`, §6.5 resolves it against whatever the lineage looks like now, and §6.6's matrix already holds the answers, including for a squash of the range containing `review.of.current`. Measured 32 producer revisions after review, 84.87% of npm threads still resolved `survives` with exact offsets and 12.32% went to the selector, where the quote placed 77 of 79 correctly against a stored offset's 48 of 79.

**Nothing is added to the reviewed package.** A conforming package is already reviewable: the four steps above read the manifest, the ledger when present, and one document, which is what §6.5 already reads. No review-specific entry, field or index appears in the reviewed package, and §4's admission rule is untouched. One existing property does matter and is already required — the current view is the working tree of `current` (§2, §3.8), so an offset minted on the reviewer's machine lands on the same character on the producer's. D-17 and D-18a are what make that true across platforms; without them every offset would be displaced by one character per preceding line on a CRLF host.
...
| Review data tracked inside the reviewed package | Changes the tip tree, hence `current`, hence the identity every comment points at: the artifact would invalidate its own references on write |
...
| A bare `review.json` beside the package as the interchange artifact | Forfeits 79-byte typing, central-directory integrity, the LF rule, the text-flag rule and any history, to save a flat 2,613–2,615 bytes on npm scale. Permitted as a raw transport payload; it is not what returning a review means |
...
- **D-23** Review data lives in a **second package** with its own manifest, namespace and curated repository, at the tracked path `.mdpkg/review/comments.json` under the §3.6 exception (§6.8). The three rejected homes and their reasons are in §9; the decisive one is that tracking review data inside the reviewed package changes `current` and so invalidates the identity every comment in it points at. Measured price of being a package rather than a bare JSON file: a flat 2,613–2,615 bytes on npm scale, independent of thread count.
- **D-24** Two review-package shapes are first-class and declared in the manifest as `review.shape` (§4): `delta`, the review alone in the reviewer's own fresh namespace, and `bundled`, the review plus the reviewed package, continuing the reviewed lineage so that the tip commit's parent is `review.of.current`. A `bundled` review commit MUST change only paths under `.mdpkg/review/`, which is what makes it safe for a reviewer to commit in the reviewed namespace without confirmation authority under §6.3, and which a validator checks by diffing the tip tree against its parent. Measured on npm: 5,098 / 37,463 bytes for 1 / 50 threads as `delta` against a 251,778-byte original, and 254,262 / 286,628 as `bundled`.
- **D-25** The correlation key from a review package to the package it reviews is the pair `(review.of.namespace, review.of.current)` and nothing else; `review.of.packageDigest` and `review.of.packageBytes` are optional corroborating evidence (§4). Measured: a re-emission at another DEFLATE level left `current` and every entry payload identical while changing the file's size and SHA-256, so a content hash reports a false mismatch on a correlation that is exact; `current` also distinguishes two packages whose trees are identical but whose retained history or commit time differs. `current` matching while the digest does not is a fact worth reporting, not an error. This is the identity-versus-evidence split §5.5 already makes for archived patches and §6.4 makes for `root` versus `expect`.
