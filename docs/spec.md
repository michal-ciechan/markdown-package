...
The first three were deferred by container.md and are not decided here; the fourth is a decision this document did take, recorded here because measuring it raised two questions this document does not answer.
...
4. **EOL behaviour, measured: D-18 holds for the unpack and not for the checkout after it.** D-17 and D-18 were decided on argument alone; [2026-09-08-card-0010-eol-evidence.md](investigations/2026-09-08-card-0010-eol-evidence.md) measured them against a CRLF source and every ZIP tool on a Windows host. D-17 is confirmed and is heavier than its own wording: projecting a three-commit CRLF source rewrote 7 of 10 blob slots, all 3 trees and all 3 commits with no history transformation of any kind, while the same projection of an LF source was the exact identity — so it is this rule, not projection in general, that severs a package's commit IDs from its source. The §4 validator obligation is confirmed too: a CRLF-storing package and its conforming twin differ in size, SHA-256, `HEAD` and 3 of 4 document CRCs and resolve to the same 15 digests, and the EOL scan that separates them costs only payload the validator has already decoded. At the unpack layer D-18 held in all nine invocations — not one added a CR byte, and `unzip -aa`, the only converting mode, damaged the pack and index rather than the documents (`git fsck` exit 70). What is not confirmed is the Git half of §3.8. `.git/config` (§5.1) declares no `core.autocrlf`, so the host's setting governs; under Git for Windows' installer default `core.autocrlf=true`, `git read-tree HEAD` and `git status` stay byte-exact and clean, and the next `git checkout -- .` or `git clone` of the extraction rewrites every document to CRLF. Digests still resolve, because §6.1 rule 1 re-normalizes; the byte-for-byte property D-18 exists to give does not survive. Two questions follow and neither is decided here: whether §5.1's fixed `.git/config` bytes should pin `core.autocrlf` (measured to work, but it is a filter configuration §3.8 currently says the format ships none of), and whether §3 should state the ZIP internal-attributes value — a conforming package is immune to `unzip -a` only because the producer flags every entry binary, which is why §2's parenthetical naming that command as a counterexample is not reproduced against a conforming package.
