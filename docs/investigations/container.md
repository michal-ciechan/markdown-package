...
One field this card left at its assembler default has since become normative. Every central-directory record the fixture writes has internal file attributes `0` — the ZIP text flag clear — and [CARD-0010 §4.1](2026-09-08-card-0010-eol-evidence.md#41-why-unzip--a-is-a-no-op-and-what-that-depends-on) measured that this, not the EOL rule on its own, is what makes a conforming package a byte-exact no-op under Info-ZIP `unzip -a`: across four combinations of stored content against that bit, conversion happens in exactly one cell, CRLF content in a text-flagged archive, and strips 21 CR bytes from 3 of 4 documents. spec.md **D-19** now requires the field to be `0` on every entry and §4 makes a validator check it, so extraction safety rests on two independent rules rather than on D-17 alone.
...
Neither step touches content bytes: a ZIP unpack writes the entry payload verbatim, and `git read-tree` only builds an index from the tree already on disk. Nothing here plays the role of Git's `core.autocrlf` smudge filter on checkout, and the package ships no `.gitattributes` to configure one, so the extracted files are exactly the LF-normalized bytes D-17 stored — on Windows as much as anywhere else (spec.md D-18a). That is `core.autocrlf=input` behavior (normalize on the way in, do nothing on the way out), not `core.autocrlf=true` (also convert to CRLF on checkout). The alternative was not measured because it does not exist for this container as specified: platform-native checkout is a filter Git itself only applies through configuration this format does not ship, and adding one would cost the extracted file's one useful property — that it is the tracked blob byte for byte, so a reader can hash it under CARD-0004's profile without first undoing what checkout did to it.

That claim is measured, and it is exactly as wide as the procedure this paragraph describes: spec.md D-18 has since been split, and what is stated above is **D-18a**. [CARD-0010](2026-09-08-card-0010-eol-evidence.md#5-d-18-second-half-git-read-tree-and-what-a-consumer-does-next) ran nine real extractor invocations, none of which added a CR byte in any region, and confirmed that `git read-tree HEAD` writes only the index and leaves the working tree byte-identical under every `core.autocrlf` setting — including `true`, where the clean filter finds LF content already at its fixed point and reports nothing modified. What this paragraph does not cover, because the two-step procedure stops there, is the *next* Git command. The shipped `.git/config` (spec.md §5.1) declares no `core.autocrlf`, so the host's setting governs the extracted repository, and under Git for Windows' installer default `core.autocrlf=true` the following `git checkout -- .` or `git clone` rewrites every document to CRLF: 0 of 4 byte-identical, 25 CR bytes added by each. spec.md **D-18b** records that as intended rather than as a defect — the host keeps its own line-ending policy, the package pins nothing (a `core.autocrlf` pin in `.git/config` was measured, works, and is rejected in spec.md §9), and the byte-for-byte property is a guarantee of the unpack rather than of a working tree a consumer has since checked out. Digests are unaffected either way, because CARD-0004's profile re-normalizes on read.
