...
- Name uniqueness uses NFC followed by Unicode 17.0.0 **simple** case folding
  (C + S mappings, no full or Turkic folding). The spec leaves the Unicode
  version unstated; the reader pins it for repeatability. The checked-in table
...
including reserved-slot births. A resolved reference displays its status,
...
Loose references to packages with partial correspondence coverage return
